...
public class AuctionDAO{
	...
	public List<Auction> findSellerClosedAuctions(int userID) throws SQLException{
		...
		String query = "SELECT auction.auctionid, auction.startingbid, auction.minraise, auction.deadline, COALESCE(MAX(bid.price)) AS bestbid FROM auction LEFT JOIN bid ON auction.auctionid=bid.auctionid WHERE auction.creatorid=? AND auction.isopen=? GROUP BY auction.auctionid, auction.startingbid, auction.minraise, auction.deadline ORDER BY auction.deadline ASC";
	...
	public Auction searchDatas(int auctionid) throws SQLException{
		...
		String query = "SELECT deadline, minraise, COALESCE(MAX(bid.price), 0) as p FROM auction LEFT JOIN bid ON auction.auctionid = bid.auctionid WHERE auction.auctionid=?";
        ...
        try {
	        ...
	        if (!resultSet.isBeforeFirst()) { // no results, credential check failed
				...
				System.out.println(resultSet.getFloat("p") +"--"+ resultSet.getFloat("minraise")+"--"+ resultSet.getTimestamp("deadline"));
		...
		try {
			...
			if (resultSet.next()) {
			    ...
			    while (resultSet.next()) {
			        ...
			        if(!auction.getItemList().contains(item))
			        	auction.setItem(item);
			        item=null;
			    } 
			} 
	...
	public Auction getAuction(int auctionid) throws SQLException {
		String query = "select auctionid, creatorid, isopen, deadline from auction where auctionid = ? ";
